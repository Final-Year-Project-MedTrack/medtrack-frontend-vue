<template>
    <div class="p-6 bg-white ">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-semibold text-gray-800">Medical Records With Lab Result</h2>
        </div>
        ...
        <div v-else-if="patientVisitLaboratoryTests.length == 0">
        ...
        <div v-else v-for="(patientVisitLaboratoryTest, index) in patientVisitLaboratoryTests" :key="index"
            class="mb-4 border border-gray-100 rounded-lg">
            <div class="p-4 flex justify-between items-center cursor-pointer" @click="toggle(index)">
                <div>
                    <h2>{{ patientVisitLaboratoryTest.laboratoryTest.name }}</h2>
                    <p class="text-sm text-gray-500">{{ patientVisitLaboratoryTest.created_at }}</p>
            ...
            <div class="py-12">
                <div class="right w-auto">
                    ...
                    <router-link :to="{
                        ...
                        params: {
                            'visitId': visit.id,
                            'visitLabTestId': patientVisitLaboratoryTest.id
                        }
...
const patientVisitLaboratoryTests = ref([])
...
onMounted(async () => {
    try {
        const response = await api.get(
            `medical-provider/patient-visit/patient-visit-laboratory-test/`, {
            params: {
                visit_id: props.visit.id,
            }
        })
        patientVisitLaboratoryTests.value = response.data.data.items
    } catch (err) {
        error.value = 'Failed to fetch Visit Lab Test Information'
        console.error(err)
    } finally {
        loading.value = false
    }
