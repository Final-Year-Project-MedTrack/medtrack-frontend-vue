    ...
    <div class="space-y-4">
      ...
      <div v-else v-for="(visit, index) in visits" :key="index" class="bg-gray-50 rounded-md p-4 space-y-2">
        ...
        <div class="border-t border-gray-100 flex justify-between w-full">
          ...
          <div class="">
            <router-link :to="{'name': 'RadiologistDashboardViewVisitWithLabResults', 'params': {'visitId': visit.id}}" class=" border border-gray-600 rounded p-2 my-4 right-0 text-sm text-gray-600 hover:underline">
              View medical record →
            </router-link>
          </div>
        </div>
